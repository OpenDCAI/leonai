...
async def _create_agent(**kwargs):
    return await asyncio.to_thread(create_leon_agent, **kwargs)
...
class TestE2BRealE2E:
    ...
    async def test_e2b_basic_execution(self):
        ...
        agent = await _create_agent(sandbox="e2b")

        # Execute real command
        result = await agent._sandbox.shell().execute("echo 'E2B Real Test'")
    ...
    async def test_e2b_terminal_state_persistence(self):
        ...
        # Change directory
        await agent._sandbox.shell().execute("cd /tmp")
        result = await agent._sandbox.shell().execute("pwd")
        assert "/tmp" in result.stdout

        # Set environment variable
        await agent._sandbox.shell().execute("export E2B_TEST_VAR=real123")
        result = await agent._sandbox.shell().execute("echo $E2B_TEST_VAR")
    ...
    async def test_e2b_file_operations(self):
        ...
        await agent._sandbox.shell().execute(f"echo '{test_content}' > /tmp/e2b_real_test.txt")

        # Read file
        content = agent._sandbox.fs().read_file("/tmp/e2b_real_test.txt")
        assert test_content in content.content

        # List directory
        files = agent._sandbox.fs().list_dir("/tmp")
        assert any("e2b_real_test.txt" in str(f) for f in files.entries)
    ...
    async def test_e2b_multiple_commands_state_tracking(self):
        ...
        await agent._sandbox.shell().execute("cd /home")
        await agent._sandbox.shell().execute("export VAR1=value1")
        await agent._sandbox.shell().execute("export VAR2=value2")

        result = await agent._sandbox.shell().execute("pwd && echo $VAR1 && echo $VAR2")
...
class TestAgentBayRealE2E:
    ...
    async def test_agentbay_basic_execution(self):
        ...
        agent = await _create_agent(sandbox="agentbay")

        # Execute real command
        result = await agent._sandbox.shell().execute("echo 'AgentBay Real Test'")
        assert result.exit_code == 0
    ...
    async def test_agentbay_terminal_state_persistence(self):
        ...
        # Set environment variable
        await agent._sandbox.shell().execute("export AGENTBAY_TEST_VAR=real456")
        result = await agent._sandbox.shell().execute("echo $AGENTBAY_TEST_VAR")
    ...
    async def test_agentbay_file_operations(self):
        ...
        await agent._sandbox.shell().execute(f"echo '{test_content}' > /tmp/agentbay_real_test.txt")

        # Read file
        content = agent._sandbox.fs().read_file("/tmp/agentbay_real_test.txt")
        assert test_content in content.content
...
class TestDaytonaRealE2E:
    ...
    async def test_daytona_basic_execution(self):
        ...
        agent = await _create_agent(sandbox="daytona")

        # Execute real command
        result = await agent._sandbox.shell().execute("echo 'Daytona Real Test'")
        assert result.exit_code == 0
    ...
    async def test_daytona_terminal_state_persistence(self):
        ...
        # Set environment variable
        await agent._sandbox.shell().execute("export DAYTONA_TEST_VAR=real789")
        result = await agent._sandbox.shell().execute("echo $DAYTONA_TEST_VAR")
