...
class SupabaseThreadConfigRepo:
    ...
    def lookup_config(self, thread_id: str) -> dict[str, str | None] | None:
        rows = self._select(thread_id, "sandbox_type,cwd,model,queue_mode,observation_provider,agent,bind_mounts_json")
        if not rows:
            return None
        ...
        return {
            ...
            "bind_mounts_json": str(rows[0]["bind_mounts_json"]) if rows[0].get("bind_mounts_json") is not None else None,
        }
