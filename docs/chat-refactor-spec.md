...
**实时流（SSE）**：`new_run` 和 `run_done` 事件分别标记 Turn 的开始和结束。
...
26 种 SSE 事件，通过 type 前缀区分来源（`task_text`、`subagent_task_text`、`background_task_text`），本质是同一件事的重复。Task 和 SubAgent 语义完全重叠，唯一区别是 SubAgent 多一个 `parent_tool_call_id`。
...
// 改后
{ type: "tool_call", data: { agent_id: "task_abc", parent_id: "tc_xyz", ... } }
```
...
| 类型 | 事件 | 说明 |
|------|------|------|
| 内容 | `text` `tool_call` `tool_result` `error` `cancelled` | 5 种，靠 agent_id 区分来源 |
| 生命周期 | `task_start` `task_done` `task_error` | 3 种，覆盖 Turn 内子 Agent 和 Background Task |
| 控制 | `status` `new_run` `run_done` | 3 种 |
...
主 Agent 事件：`agent_id = "main"`，无 `parent_id`。
子 Agent / Background Task 事件：`agent_id = task_id`，`parent_id = parent_tool_call_id`（可选），`background: true/false` 区分是否后台任务。

26 种 → 11 种。前端收到事件后，先看 `type` 决定怎么渲染，再看 `agent_id` / `parent_id` 决定往哪个 Turn / ToolStep 里挂。Background Task 的 `task_done` 只是触发前端 re-fetch Task Output API，不携带完整数据。
...
→ [
    { task_id, type: "bash",  label: "npm test",  status: "running", started_at },
    { task_id, type: "agent", label: "代码审查",   status: "done",    started_at },
  ]
...
| SSE 事件类型 | 26 种，前缀区分来源 | 11 种，靠 agent_id/parent_id 路由 |
| Task/SubAgent | 两套平行事件 | 统一为一套，靠 parent_id 有无区分 |
